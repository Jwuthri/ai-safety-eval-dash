#!/bin/bash
# Deploy script for ai-safety-eval-dash Backend

set -e

echo "🚀 ai-safety-eval-dash Backend Deployment"
echo "==================================================="
...
backup_deployment() {
    if [ "$BACKUP" = "true" ]; then
        ...
        if docker ps | grep -q "ai-safety-eval-dash_postgres"; then
            log_info "Backing up PostgreSQL database..."
            docker exec ai-safety-eval-dash_postgres pg_dump -U postgres ai-safety-eval-dash > "$BACKUP_DIR/database.sql"
        fi
        

        # Backup volumes
        log_info "Backing up Docker volumes..."
        docker run --rm -v ai-safety-eval-dash_redis_data:/data -v $(pwd)/$BACKUP_DIR:/backup alpine tar czf /backup/redis_data.tar.gz -C /data .
...
# Deploy application
deploy_application() {
    log_info "Deploying ai-safety-eval-dash Backend..."
...
post_deployment() {
    ...
    if [ -n "$DEPLOYMENT_WEBHOOK_URL" ]; then
        ...
        curl -X POST "$DEPLOYMENT_WEBHOOK_URL" \
            ...
            -d "{\"text\":\"ai-safety-eval-dash Backend deployed successfully to $ENVIRONMENT!\"}" \
    ...
    if [ -d "./backups" ]; then
        ...
        if [ -n "$LATEST_BACKUP" ]; then
            ...
            if [ -f "./backups/$LATEST_BACKUP/database.sql" ]; then
                ...
                sleep 10
                docker exec -i ai-safety-eval-dash_postgres psql -U postgres ai-safety-eval-dash < "./backups/$LATEST_BACKUP/database.sql"
            fi
            ...
            if [ -f "./backups/$LATEST_BACKUP/redis_data.tar.gz" ]; then
                ...
                docker run --rm -v ai-safety-eval-dash_redis_data:/data -v $(pwd)/backups/$LATEST_BACKUP:/backup alpine tar xzf /backup/redis_data.tar.gz -C /data
            fi
